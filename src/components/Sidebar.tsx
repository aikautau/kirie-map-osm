...
import { AddIcon, DeleteIcon, EditIcon, MapIcon, MenuIcon, CloseIcon } from './Icons';
...
interface SidebarProps {
  ...
  onUpdateRecord: (title: string, year?: string, address?: string, regionType?: 'domestic'|'overseas', prefecture?: string, countryCode?: string, mapNumber?: number, mapNumberText?: string) => void;
...
const EditRecordForm: React.FC<{ 
  ...
  const [regionType, setRegionType] = useState<'domestic'|'overseas'>((record as any).regionType || 'domestic');
  const [prefecture, setPrefecture] = useState<string>((record as any).prefecture || '東京都');
  const [countryCode, setCountryCode] = useState<string>((record as any).countryCode || 'JP');
  const [mapNumberStr, setMapNumberStr] = useState<string>((record as any).mapNumber ? String((record as any).mapNumber) : '');
  const [mapNumberText, setMapNumberText] = useState<string>((record as any).mapNumberText || '');
  ...
  return (
    <div className="p-4 bg-gray-700/50 backdrop-blur-sm rounded-lg mt-4">
      <h3 className="text-lg font-semibold text-white mb-2">Edit Record</h3>
      <p className="text-sm text-gray-300 mb-3">Modify the record details and map area if needed.</p>
      <form onSubmit={handleSubmit}>
        <input
          type="text"
          value={title}
      ...
      `}>
        <div className="p-4 border-b border-gray-700 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-300 mt-1 ml-8">線はどこからきたのか</p>
          </div>
          <button
        ...
        <div className="flex-grow overflow-y-auto p-4">
          ...
          {appMode === 'admin' && !isAdding && !isEditing && (
            <button
